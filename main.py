...
class Pingo:
    def __init__(self):
        ...
        self.asr = XunfeiVoice(Xunfei_APP_ID, Xunfei_API_KEY, Xunfei_SECRET_KEY)
        self.tts = EdgeVoice(voice_name=conf().get("voice_name","zh-CN-YunjianNeural"))

        # 创建插件功能
        self.chat_module = bgunit(self.tts)

    def run(self):

        self.tts.text_to_speech_and_play(
        ...
        try:
            while True:
                ...
                if result >= 0:
                    ...
                    print("我在,请讲！")
                    self.tts.text_to_speech_and_play("我在,请讲！")

                    num = 3  # 最多循环确认4次
                    self.chat_module.begin()
                    while not self.chat_module.conversation_is_complete() and num > 0:
                        num = num - 1
                        q = self.asr.speech_to_text()
                        logger.info("recognize_from_microphone, text= %s", q)
                        # 调用插件功能，进行意图处理
                        self.chat_module.chat_with_unit(q)

                    self.chat_module.end()
            ...
            logger.error(
                "[Porcupine] AccessKey PICOVOICE_API_KEY refused" , stack_info=True
            )
            raise e
        except pvporcupine.PorcupineActivationThrottledError as e:
            logger.error(
                "[Porcupine] AccessKey PICOVOICE_API_KEY has been throttled",
                stack_info=True,
            )
            raise e
            ...
            self.tts.text_to_speech_and_play("好的，我退下了，再见！")
